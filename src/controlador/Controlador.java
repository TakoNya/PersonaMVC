...
public class Controlador implements ActionListener{
    ...
    public Controlador(Vista v) {
        ...
        this.vista.tabla.addMouseListener(new MouseAdapter(){
            public void mouseClicked(MouseEvent evnt) {
                if(evnt.getClickCount() == 1) { editar(); }
            }} );
    }
    @Override
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource() == vista.btnGuardar) {
            if(vista.txtId.getText().isEmpty()) { agregar(); } else {actualizar();}
            listar(vista.tabla);
            nuevo();
        }
    ...
    public void borrar() {
        ...
        if(fila==-1){
            ...
            if(input == 0) {
                int id = Integer.parseInt((String) vista.tabla.getValueAt(fila,0).toString());
                dao.Delete(id);
                System.out.println("Se ha borrado la persona con éxito (" + id + ")");
            }
        }
        limpiarTabla();
    }
    public void agregar(){
        ...
        int r = dao.agregar(p);
        if (r == 1) {
            System.out.println("Se ha agregado la persona con éxito.");
        }else { JOptionPane.showMessageDialog(vista, "Error"); }
        limpiarTabla();
    }
    public void actualizar(){
        if(vista.txtId.getText().equals("")){
            JOptionPane.showMessageDialog(vista, "Para actualzar debe seleccionar primero la opción Editar");
        }else {
            ...
            if ( r == 1) {
                System.out.println("Se ha actualizado la persona con éxito(" + id + ")");
            }else { JOptionPane.showMessageDialog(vista,"Error");}
        }
        limpiarTabla();
    }
    ...
    void centrarCeldas(JTable tabla) {
        ...
        for(int i =0; i < vista.tabla.getColumnCount(); i++) {tabla.getColumnModel().getColumn(i).setCellRenderer(tcr);}
